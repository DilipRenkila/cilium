...
var _ = Describe("K8sHubbleTest", func() {
	...
	Context("Hubble Observe", func() {
		...
		BeforeAll(func() {
			res := kubectl.Apply(helpers.ApplyOptions{FilePath: demoPath, Namespace: ns})
			res.ExpectSuccess("could not create demo deployment")

			err := kubectl.WaitforPods(ns, "-l zgroup=testapp", helpers.HelperTimeout)
			Expect(err).Should(BeNil(), "Test pods are not ready after timeout")

			appPods = helpers.GetAppPods(apps, helpers.DefaultNamespace, kubectl, "id")

			app1ClusterIP, app1Port, err = kubectl.GetServiceHostPort(ns, app1Service)
			Expect(err).To(BeNil(), "Cannot get service in %q namespace", ns)
		...
		It("Test L3/L4 Flow", func() {
			...
			observe := hubbleObserve(ctx, ciliumPodK8s1, fmt.Sprintf(
				"--follow --last 1 --json --type trace --from-pod %s/%s --to-label %s --to-port %d",
				ns, appPods[helpers.App2], app1Labels, app1Port))

			res := kubectl.ExecPodCmd(ns, appPods[helpers.App2],
		...
		It("Test L7 Flow", func() {
			...
			res := kubectl.Exec(fmt.Sprintf("%s annotate pod -n %s -l %s %s=\"<Ingress/80/TCP/HTTP>\"", helpers.KubectlCmd, ns, app1Labels, annotation.ProxyVisibility))
			...
			defer cancel()
			observe := hubbleObserve(ctx, ciliumPodK8s1, fmt.Sprintf(
				"--follow --last 1 --json --type l7 --from-pod %s/%s --to-label %s --protocol http",
				ns, appPods[helpers.App2], app1Labels))
			...
			res = kubectl.Exec(fmt.Sprintf("%s annotate pod -n %s -l %s %s-", helpers.KubectlCmd, ns, app1Labels, annotation.ProxyVisibility))
